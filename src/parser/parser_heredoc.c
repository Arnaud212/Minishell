...
/*   Updated: 2023/08/16 08:42:22 by jchauvet         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static char	*heredoc_help(char *read_str, char *here_str);
...
static char	*heredoc_readline(char *limiter)
{
	...
	{
		if (test_sig(read_str, 0))
			break ;
		read_str = str_append_chr(read_str, '\n');
		if (read_str == NULL)
			return (free_and_print(here_str));
	...
	here_str = heredoc_help(read_str, here_str);
	return (here_str);
}
